...
import { criteriaGroups } from "@/lib/mock-data";

export const Route = createFileRoute("/new-review")({
  head: () => ({ meta: [{ title: "New Review — UXNavigator" }] }),
  component: NewReview,
});
...
function NewReview() {
  ...
  const [files, setFiles] = useState([
    { name: "onboarding-step-1.png", type: "Screenshot", status: "Uploaded" },
    { name: "PRD-Onboarding-v3.pdf", type: "PRD", status: "Uploaded" },
  ]);
  ...
  const runReview = () => {
    ...
    setStageIdx(0);
    const tick = (i: number) => {
      if (i >= progressStages.length) {
        toast.success("Review complete. Opening workspace…");
        setTimeout(() => navigate({ to: "/workspace" }), 600);
        return;
      }
      setStageIdx(i);
            ...
            <li key={s} className="flex items-center gap-2">
              <button
                ...
                className={cn(
                  ...
                  : i < step ? "border-[color:var(--success)]/40 bg-[color:var(--success)]/10 text-[color:var(--success)]"
                  : "border-border bg-card text-muted-foreground hover:bg-secondary",
                )}
              >
                <span className={cn("flex h-5 w-5 items-center justify-center rounded-full text-[10px]",
                  i === step ? "bg-primary-foreground/20" : i < step ? "bg-[color:var(--success)]/20" : "bg-secondary")}>
            ...
            <CardContent className="space-y-6">
              {step === 0 && (
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <Field label="Reviewer / owner" className="md:col-span-2">
                    <Input defaultValue="Rakhee Sharma" />
                  </Field>
                </div>
              )}
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <div className="rounded-xl border-2 border-dashed border-border bg-secondary/40 p-8 text-center transition hover:border-primary/40 hover:bg-secondary/70">
                    ...
                    <Button variant="outline" size="sm" className="mt-3 min-h-9">Browse files</Button>
                  </div>
                  ...
                  <Field label="Flow screenshots & PRD">
                    <Textarea rows={3} placeholder="Paste flow notes, PRD excerpts, or context the AI should know." />
                  </Field>
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      {files.map((f) => (
                        <div key={f.name} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3">
                          ...
                          <button onClick={() => setFiles(files.filter((x) => x.name !== f.name))} className="rounded p-1 text-muted-foreground hover:text-destructive" aria-label={`Remove ${f.name}`}>
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                      ))}
              ...
              {step === 3 && (
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <Field label="Areas included" help="Key flows only narrows the review to the main user journeys and skips peripheral screens.">
                  ...
                  <div className="space-y-2.5 md:col-span-2">
                    ...
                    {[
                      ["Recommendations with acceptance criteria", "Actionable fixes written as testable criteria.", true],
                      ["Linked principle for each finding", "Cites the UX heuristic or principle the issue violates.", true],
                      ["Requirement traceability", "Maps the finding back to the relevant PRD or requirement.", true],
                      ["Accessibility impact (WCAG)", "Adds WCAG severity and context notes.", true],
                      ["Business impact estimate", "Estimates user impact and conversion risk.", true],
                    ].map(([label, tip, def]) => (
                      <div key={label as string} className="flex items-center justify-between rounded-lg border border-border bg-card px-3 py-2.5">
                        <Label className="flex items-center gap-1.5 text-sm">
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button type="button" className="rounded p-0.5 text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Help for ${label}`}>
                        ...
                        <Switch defaultChecked={def as boolean} aria-label={label as string} />
                      </div>
                    ))}
                  </div>
                  <div className="md:col-span-2 flex gap-2 rounded-lg border border-[color:var(--info)]/30 bg-[color:var(--info)]/5 p-3 text-xs">
                    <Info className="h-4 w-4 shrink-0 text-[color:var(--info)]" aria-hidden="true" />
                ...
                <div className="space-y-5">
                  {!running ? (
                    <div className="rounded-xl border border-border bg-gradient-to-br from-secondary/60 to-card p-6 text-center">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">{files.length} inputs · {criteria.length} criteria · {depth} depth · ~3 min estimated</p>
                    ...
                    <div className="space-y-3 rounded-xl border border-border bg-card p-5" role="status" aria-live="polite">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-medium">AI agent at work…</p>
                      </div>
                      <Progress value={((stageIdx + 1) / progressStages.length) * 100} className="h-1.5" />
                      <ul className="mt-2 space-y-1.5">
                        {progressStages.map((s, i) => (
                          <li key={s} className="flex items-center gap-2 text-sm">
                            {i < stageIdx ? <Check className="h-3.5 w-3.5 text-[color:var(--success)]" aria-hidden="true" />
                ...
                <CardContent className="flex flex-wrap gap-1.5">
                  {criteria.map((c) => (
                    <Badge key={c} variant="secondary" className="gap-1 pl-2 pr-1 text-[10px]">
                      {c}
                      <button onClick={() => toggleCriterion(c)} className="ml-1 rounded-full p-0.5 hover:bg-foreground/10" aria-label={`Remove ${c}`}>
                        <X className="h-3 w-3" />
                      </button>
                    </Badge>
                  ))}
